...
(function(jq){
	...
	posterTvGrid.prototype = {
		reset: function(options){
			...
			if(!this.options.width){
				if(parseInt(this.body.outerWidth())>1255 || navigator.userAgent.indexOf('iPad') !== -1){
					...
					this.pageNowLeft = this.options.curLeft;
				}else{
					this.options.width = 970;
					this.pageNowLeft = 300;
				}
			}else{
				if(parseInt(this.body.outerWidth())>1255 || navigator.userAgent.indexOf('iPad') !== -1){
					this.pageNowLeft = this.options.curLeft;
			...
			this.preLeft = 0;
			this.nextLeft = this.options.width-this.options.imgWidth;
			this.preNLeft = -this.options.imgWidth;
		...
		initContent: function(){
			...
			if(this.total > 2){
				...
				this.parent.find(".contentHolderUnit:nth-child("+this.pageNow+")").css({width:this.options.imgWidth+'px',height:this.options.imgCurHeight+'px', opacity: 1, left: this.pageNowLeft+'px', zIndex: 3, marginTop: 0});
				...
				contentHolderUnit.css({width:'50%',height:'0px', opacity: 0, left:'0', zIndex:0, marginTop: '135px', textAlign:'center'});
				this.parent.find(".contentHolderUnit:nth-child("+this.pageNow+")").css({height:this.options.imgHeight+'px', opacity: 1, zIndex: 3, marginTop: '23px'});
			}else{
				contentHolderUnit.css({height:'0px', width:'100%', opacity: 0, left:this.options.width/2+'px', zIndex:0, marginTop: '135px'});
				this.parent.find(".contentHolderUnit:nth-child("+this.pageNow+")").css({height:this.options.imgHeight+'px', opacity: 1, left: '0', zIndex: 3, marginTop: '23px', textAlign:'center'});
				//this.parent.find(".contentHolderUnit:nth-child("+this.pageNow+") .elementOverlay").css({opacity:0});
				//this.parent.find(".contentHolderUnit:nth-child("+this.pageNow+") .leftShadow").css({opacity:1});
				//this.parent.find(".contentHolderUnit:nth-child("+this.pageNow+") .rightShadow").css({opacity:1});
			}
			...
			if(this.total > 2){
				this.parent.find(".contentHolderUnit:nth-child("+pre+")").css({opacity: 1, left: this.preLeft+'px',height: this.options.imgHeight+'px', width: this.options.imgWidth+'px', zIndex: 0, marginTop: '23px'});
				...
				this.parent.find(".contentHolderUnit:nth-child("+next+")").css({opacity: 1, left: this.nextLeft+'px',height: this.options.imgHeight+'px', width: this.options.imgWidth+'px', zIndex: 0, marginTop: '23px'});
				...
				this.parent.find(".contentHolderUnit:nth-child("+next+")").css({opacity: 1, left: '50%', height: this.options.imgHeight+'px', zIndex: 0, marginTop: '23px'});
			}else{
				this.parent.find(".contentHolderUnit:nth-child("+next+")").css({opacity: 1, left: '0',height: this.options.imgHeight+'px', zIndex: 0, marginTop: '23px'});
				//this.parent.find(".contentHolderUnit:nth-child("+next+") .elementOverlay").css({opacity:0.4});
				//this.parent.find(".contentHolderUnit:nth-child("+next+") .leftShadow").css({opacity:0});
				//this.parent.find(".contentHolderUnit:nth-child("+next+") .rightShadow").css({opacity:0});
			}
		...
		turnpage: function(page, dir){
			...
			var run = function(page, dir, t){
				...
				if(dir == 'left'){					
					...
					_this.parent.find(".contentHolderUnit:nth-child("+pre+")").animate({opacity: 1, left: _this.preLeft+'px', height: _this.options.imgHeight+'px', width: _this.options.imgWidth+'px', zIndex: 2, marginTop: '23px'}, t);
					...
					_this.parent.find(".contentHolderUnit:nth-child("+page+")").animate({opacity: 1, left: _this.pageNowLeft+'px', height: _this.options.imgCurHeight+'px', width: 'auto', zIndex: 3, marginTop: '0'}, t);
					...
					_this.parent.find(".contentHolderUnit:nth-child("+next+")").css({opacity: 0, left: _this.nextNLeft+'px', height: '100px', width: _this.options.imgWidth+'px', zIndex: 2, marginTop: '85px'});
					_this.parent.find(".contentHolderUnit:nth-child("+next+")").animate({opacity: 1, left: _this.nextLeft+'px', height: _this.options.imgHeight+'px', width:_this.options.imgWidth+'px', zIndex: 2, marginTop: '23px'}, t);
					...
					_this.parent.find(".contentHolderUnit:nth-child("+preP+")").animate({width:_this.options.imgWidth+'px',height:'100px', opacity: 0, left:_this.preNLeft+'px', zIndex:0, marginTop: '85px'},t, "", function(){_this.locked=false;});
					...
					_this.parent.find(".contentHolderUnit:nth-child("+page+")").animate({opacity: 1, left: _this.pageNowLeft+'px', height: _this.options.imgCurHeight+'px', width: 'auto', zIndex: 3, marginTop: '0px'}, t);
					//_this.parent.find(".contentHolderUnit:nth-child("+page+") .elementOverlay").css({opacity:0});
					//_this.parent.find(".contentHolderUnit:nth-child("+page+") .leftShadow").css({opacity:1});
					//_this.parent.find(".contentHolderUnit:nth-child("+page+") .rightShadow").css({opacity:1});
					
					_this.parent.find(".contentHolderUnit:nth-child("+pre+")").css({opacity: 0, left: _this.preNLeft+'px', height: '100px', width: _this.options.imgWidth+'px', zIndex: 2, marginTop: '85px'});
					_this.parent.find(".contentHolderUnit:nth-child("+pre+")").animate({opacity: 1, left: _this.preLeft+'px', height: _this.options.imgHeight+'px', width: _this.options.imgWidth+'px', zIndex: 2, marginTop: '23px'}, t);
					//_this.parent.find(".contentHolderUnit:nth-child("+pre+") .elementOverlay").css({opacity:0.4});
					//_this.parent.find(".contentHolderUnit:nth-child("+pre+") .leftShadow").css({opacity:0});
					//_this.parent.find(".contentHolderUnit:nth-child("+pre+") .rightShadow").css({opacity:0});
					...
					_this.parent.find(".contentHolderUnit:nth-child("+nextN+")").animate({width:_this.options.imgWidth+'px',height:'100px', opacity: 0, left:_this.nextNLeft+'px', zIndex:0, marginTop: '85px'}, t, "",function(){_this.locked=false;});
				}
	...
	posterTvGrid.options = {
		...
		width : 1100,//最外层宽，需要使用的时候在传,默认由程序自动判断
		height : 360,//最外层高  
		delay : 2000,//滚动间隔（毫秒）
		speed : 500, //滚动速度毫秒
		imgCurHeight: 320, //当前居中图片的默认高度
		imgHeight: 270, // 图片默认的高度
		imgWidth: 400, //图片的默认宽度
		curLeft: 395 //当前居中图片的left值
	};
